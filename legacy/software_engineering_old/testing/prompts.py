...
class TestingPrompts(BaseSEPrompts):
    """Prompts specialized for the 'testing' subtopic."""

    TOOL_EXTRACTION_SYSTEM = (
        "You are an expert software engineer focusing on testing. "
    ...
    TOOL_ANALYSIS_SYSTEM = (
        "You are analyzing testing resources for professional developers. "
        "Focus on practical usage, strengths, weaknesses, and when to use them."
    )

    RECOMMENDATIONS_SYSTEM = (
        "You are a staff-level engineer advising a team on testing choices. "
    ...
    def tool_analysis_user(self, resource_name: str, content: str) -> str:
        ...
        ).format(resource_name=resource_name, content=content[:2500])
    ...
    def recommendations_user(self, query: str, resource_data: str) -> str:
        return (
            ...
            f"for improving testing. Mention:\n"
        ...
        ).format(query=query, resource_data=resource_data[:4000])
