// static/main.ts
import {DROPDOWN_OPTIONS_BY_ID} from "../advanced_agent/static/configs.js";


type Sender = "user" | "bot" | "bot-first" | "thinking" | "greeting";

interface ChatApiResponse {
    reply: string;
    download_url?: string | null;
    topic_used?: string | null; // label from /chat (optional)
    logs?: string[];
}

interface TopicApiResponse {
    topic_key: string;   // e.g. "database"
    topic_label: string; // e.g. "Databases & Data Platforms"
}
...
class ChatUI {
    ...
    private mapLanguageValue(value: string): string {
    ...
    private refreshDropdownLabels(): void {
    ...
    private addGreeting(language: string): void {
    ...
    private addDropDown(selectId: string): HTMLSelectElement | null {
        ...
        for (const opt of options) {
            ...
            (optionEl as HTMLOptionElement).dataset.labelKey = opt.label;
    ...
    private async handleSubmit(): Promise<void> {
        ...
        try {
            ...
            (this as any).currentEventSource = es;
